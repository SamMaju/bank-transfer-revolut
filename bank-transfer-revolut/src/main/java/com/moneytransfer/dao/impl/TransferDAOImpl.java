...
public class TransferDAOImpl implements TransferDAO{
	...
	public  TransferDAOImpl(UserTransaction transaction) {
	...
	/*public void run() {
		synchronized(lock) {
			try {
				 setResult(accountImpl.transfer(amount, sourceAccountNumber, destinationAccountNumber,currencyCode));
	...
	public int transferFund(UserTransaction transaction) {
		this.amount = transaction.getAmount();
        this.sourceAccountNumber = transaction.getFromAccountId();
        this.destinationAccountNumber = transaction.getToAccountId();
        this.currencyCode=transaction.getCurrencyCode();
		synchronized(lock) {
			try {
				 return transfer(amount, sourceAccountNumber, destinationAccountNumber,currencyCode);
			} catch (CustomException e) {
				e.printStackTrace();
			}
            }
